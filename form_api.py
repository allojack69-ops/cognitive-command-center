from flask import Blueprint, abort, jsonify, session
from sqlalchemy import select

from db import SessionLocal, Participant, Run, BenchmarkPack
from metrics import benchmark_metrics
from stability_metrics import boundary_stability_metrics
from form_geometry import (
    cross_form_profile,
    axis_deformation,
    pairwise_geometry,
)


bp = Blueprint("form_api", __name__)


OLD_PACK = "MORAL-MACHINE-GLOBAL-DIRECTION-V1"
NEW_PACK = "MORAL-MACHINE-BOUNDARY-STABILITY-V1"


def canon(label):
    return " ".join(
        (label or "unknown")
        .strip()
        .lower()
        .split()
    )


def latest_by_model(runs):
    out = {}

    for run in runs:
        out[canon(run.model_label)] = run

    return out


@bp.get("/benchmark/form-invariance.json")
def form_invariance_json():
    participant_id = session.get("participant_id")

    if not participant_id:
        abort(403)

    with SessionLocal() as db:
        participant = db.get(
            Participant,
            participant_id
        )

        old_pack_row = db.get(
            BenchmarkPack,
            OLD_PACK
        )

        new_pack_row = db.get(
            BenchmarkPack,
            NEW_PACK
        )

        if (
            not participant
            or not old_pack_row
            or not new_pack_row
        ):
            abort(404)

        old_pack = old_pack_row.pack()
        new_pack = new_pack_row.pack()

        old_runs = db.scalars(
            select(Run)
            .where(
                Run.participant_id == participant_id,
                Run.study_key == "BENCHMARK:" + OLD_PACK,
                Run.status == "completed",
            )
            .order_by(
                Run.completed_at.asc(),
                Run.created_at.asc(),
            )
        ).all()

        new_runs = db.scalars(
            select(Run)
            .where(
                Run.participant_id == participant_id,
                Run.study_key == "BENCHMARK:" + NEW_PACK,
                Run.status == "completed",
            )
            .order_by(
                Run.completed_at.asc(),
                Run.created_at.asc(),
            )
        ).all()

        old_latest = latest_by_model(old_runs)
        new_latest = latest_by_model(new_runs)

        models = sorted(
            set(old_latest)
            & set(new_latest)
        )

        if not models:
            return jsonify({
                "ok": False,
                "error": "no_models_with_both_forms",
            }), 404

        axes = []

        for item in new_pack.get("items", []):
            axis = item.get("latent_axis")

            if axis and axis not in axes:
                axes.append(axis)

        old_maps = {}
        new_maps = {}
        strict_maps = {}
        labels = {}

        for model in models:
            old_run = old_latest[model]
            new_run = new_latest[model]

            labels[model] = (
                new_run.model_label
                or old_run.model_label
                or model
            )

            old_metrics = benchmark_metrics(
                old_pack,
                old_run.answers,
            )

            old_maps[model] = {
                row["dimension"]:
                    row["reference_relation"]

                for row in old_metrics["rows"]

                if row.get("dimension")
                and row.get("reference_relation")
            }

            new_metrics = boundary_stability_metrics(
                new_pack,
                new_run.answers,
            )

            new_maps[model] = {
                row["axis"]:
                    row["modal_relation"]

                for row in new_metrics["axes"]
            }

            strict_maps[model] = {
                row["axis"]:
                    row["stable"]

                for row in new_metrics["axes"]
            }

        # ----------------------------------------
        # Per-model cross-form invariance
        # ----------------------------------------
        model_profiles = []

        for model in models:
            profile = cross_form_profile(
                old_maps[model],
                new_maps[model],
                axes,
            )

            shifts = [
                row
                for row in profile["rows"]
                if (
                    row["old"] is not None
                    and row["new"] is not None
                    and not row["invariant"]
                )
            ]

            model_profiles.append({
                "model": labels[model],
                "old_run_id": old_latest[model].id,
                "new_run_id": new_latest[model].id,

                "invariant_n":
                    profile["invariant_n"],

                "total_n":
                    profile["total_n"],

                "invariance_rate":
                    profile["invariance_rate"],

                "form_susceptibility":
                    profile["form_susceptibility"],

                "shifts":
                    shifts,
            })

        # ----------------------------------------
        # Per-axis form deformation
        # ----------------------------------------
        deformation = axis_deformation(
            old_maps,
            new_maps,
            models,
            axes,
        )

        axis_rows = []

        deep_core_n = 0

        for row in deformation:
            axis = row["axis"]

            cells = []

            old_values = []
            new_values = []

            for model in models:
                old = old_maps[model].get(axis)
                new = new_maps[model].get(axis)

                old_values.append(old)
                new_values.append(new)

                cells.append({
                    "model": labels[model],
                    "old": old,
                    "new": new,
                    "invariant": old == new,
                    "new_form_strict":
                        strict_maps[model].get(
                            axis,
                            False
                        ),
                })

            valid = (
                None not in old_values
                and None not in new_values
            )

            deep_core = (
                valid
                and all(
                    old == new
                    for old, new
                    in zip(
                        old_values,
                        new_values
                    )
                )
                and len(set(old_values)) == 1
                and len(set(new_values)) == 1
            )

            deep_core_n += int(deep_core)

            axis_rows.append({
                "axis": axis,
                "deep_core": deep_core,
                "shifted_models":
                    row["shifted"],
                "models":
                    row["models"],
                "deformation_rate":
                    row["deformation_rate"],
                "cells":
                    cells,
            })

        # ----------------------------------------
        # Model geometry before / after form
        # ----------------------------------------
        old_geometry = pairwise_geometry(
            old_maps,
            models,
            axes,
        )

        new_geometry = pairwise_geometry(
            new_maps,
            models,
            axes,
        )

        mean_invariance = (
            round(
                sum(
                    x["invariance_rate"]
                    for x in model_profiles
                )
                / len(model_profiles),
                3
            )
            if model_profiles
            else None
        )

        return jsonify({
            "ok": True,

            "participant_id":
                participant_id,

            "summary": {
                "models":
                    len(models),

                "axes":
                    len(axes),

                "deep_core_axes":
                    deep_core_n,

                "form_sensitive_axes":
                    len(axes) - deep_core_n,

                "mean_cross_form_invariance":
                    mean_invariance,

                "old_mean_pairwise_agreement":
                    old_geometry["mean_rate"],

                "new_mean_pairwise_agreement":
                    new_geometry["mean_rate"],

                "geometry_delta":
                    (
                        round(
                            new_geometry["mean_rate"]
                            - old_geometry["mean_rate"],
                            3
                        )
                        if (
                            old_geometry["mean_rate"]
                            is not None
                            and new_geometry["mean_rate"]
                            is not None
                        )
                        else None
                    ),
            },

            "models":
                model_profiles,

            "axes":
                axis_rows,

            "old_geometry":
                old_geometry,

            "new_geometry":
                new_geometry,
        })
